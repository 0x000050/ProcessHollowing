...
PEB* ReadPEB(HANDLE hProcess)
{
	DWORD dwPEBAddress = FindPEB(hProcess);
	PEB* pPEB = new PEB();
	BOOL bSuccess = ReadProcessMemory(hProcess, (LPCVOID)dwPEBAddress, pPEB, sizeof(PEB), 0);
	if (!bSuccess)
		return 0;
...
PLOADED_IMAGE ReadPEImage(HANDLE hProcess, LPCVOID lpImageBaseAddress)
{
